...
class PaymentFormTest {
    ...
    @Test
    public void CreditCardChargeTest() {
        CreditCard mockedCard = mock(CreditCard.class);

        doAnswer(invocation -> {
            System.out.println("Charged some amount from the mockedCard");
            return null;
        }).when(mockedCard).charge(anyDouble());

        PaymentForm paymentForm = new PaymentForm(mockedCard);
        paymentForm.pay(100.0);
        verify(mockedCard).charge(100.0);
    }
